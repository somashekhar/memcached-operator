...
import (
	...
	cachev1alpha1 "github.com/somashekhar/memcached-operator/api/v1alpha1"
)
...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// The purpose is check if the Custom Resource for the Kind Memcached is applied
	// on the cluster if not we return nil to stop the reconciliation
	memcached := &cachev1alpha1.Memcached{}
	err := r.Get(ctx, req.NamespacedName, memcached)
	if err != nil {
		if apierrors.IsNotFound(err) {
			// If the custom resource is not found then it usually means that it was deleted or
